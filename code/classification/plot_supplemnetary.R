...
### CAZY, Within-study cross validation，Cross-study validation ，Leave-one-study-out validation
df_cv <- read.csv("results_cazy_cv.csv")
df_cv$`Within-study cross validation (CV)` <- c("Within-study cross validation (CV)")
df_csv <- read.csv("results_cazy_csv.csv")
df_csv$`Cross-study validation (CSV)` <- c("Cross-study validation (CSV)")
df_loso <- read.csv("results_cazy_loso.csv")
...
df_loso <- read.csv("results_cazy_loso.csv")
df_loso$group <- "Full"
df_biomarker <- read.csv("results_cazy_loso_biomark.csv")
...
p <- plot_grid(p_1, p_2, align="hv", labels = c("a", "b"),
               nrow = 2, ncol=1, plot=FALSE)
ggsave(p, filename = "supplementary.pdf",width = 5, height = 7, useDingbats=FALSE)
