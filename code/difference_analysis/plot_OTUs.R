...
df_auc <- read.table('aucs.tsv',sep='\t', quote='',stringsAsFactors = FALSE, check.names = FALSE)
p.vals_PRJNA891951 <- read.csv("linda_single_study_PRJNA891951.csv",row.names = 1)
p.vals_PRJEB41443 <- read.csv("linda_single_study_PRJEB41443.csv",row.names = 1)
p.vals_PRJNA293971 <- read.csv("linda_single_study_PRJNA293971.csv",row.names = 1)
p.vals_PRJNA306884 <- read.csv("linda_single_study_PRJNA306884.csv",row.names = 1)
p.vals_PRJNA385004 <- read.csv("linda_single_study_PRJNA385004.csv",row.names = 1)
p.vals_PRJNA428736 <- read.csv("linda_single_study_PRJNA428736.csv",row.names = 1)
p.vals_PRJNA560950 <- read.csv("linda_single_study_PRJNA560950.csv",row.names = 1)
p.vals_PRJNA780023 <- read.csv("linda_single_study_PRJNA780023.csv",row.names = 1)
p.vals_all <- read.csv("combine_results.csv", row.names = 1)
...
legend1
ggsave("legend1.png", legend1, width = 5, height = 1)
...
pick_fid <- rownames(p.vals_all)[which(p.vals_all$padj < 0.05)]
df_auc <- df_auc %>% mutate(species = tax_df[df_auc$fid, "Species"][[1]])
df_auc <- df_auc %>% filter(fid %in% pick_fid)
species_name <- c("s__Eubacterium ramulus", "s__Faecalibacillus intestinalis", "s__Dysosmobacter sp000403435",          
                         "s__Mediterraneibacter torques", "f__Lachnospiraceae;g__;s__", "s__Blautia ammoniilytica", 
                         "s__Bifidobacterium faecale", "s__Mediterraneibacter faecis", "s__Blautia wexlerae")
names(species_name) <- unique(df_auc$species)
df_auc$species <- species_name[df_auc$species]
g <- df_auc %>% 
...
ggsave(p, filename = "Figure_2.pdf",width = 10, height = 10, useDingbats=FALSE)
