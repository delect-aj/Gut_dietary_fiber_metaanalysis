...
df_auc <- read.table('aucs_cazy.tsv',sep='\t', quote='',stringsAsFactors = FALSE, check.names = FALSE)
p.vals_PRJNA891951 <- read.csv("linda_single_study_cazy_PRJNA891951.csv",row.names = 1)
p.vals_PRJEB41443 <- read.csv("linda_single_study_cazy_PRJEB41443.csv",row.names = 1)
p.vals_PRJNA293971 <- read.csv("linda_single_study_cazy_PRJNA293971.csv",row.names = 1)
p.vals_PRJNA306884 <- read.csv("linda_single_study_cazy_PRJNA306884.csv",row.names = 1)
p.vals_PRJNA385004 <- read.csv("linda_single_study_cazy_PRJNA385004.csv",row.names = 1)
p.vals_PRJNA428736 <- read.csv("linda_single_study_cazy_PRJNA428736.csv",row.names = 1)
p.vals_PRJNA560950 <- read.csv("linda_single_study_cazy_PRJNA560950.csv",row.names = 1)
p.vals_PRJNA780023 <- read.csv("linda_single_study_cazy_PRJNA780023.csv",row.names = 1)
p.vals_all <- read.csv("combine_results_cazy.csv", row.names = 1)
...
g1 <- tibble(species=factor(rownames(p.vals.plot), 
  ...
  geom_hline(yintercept = -log10(0.1), linetype="solid", color="red") +  # 添加红线
  annotate("text", x = 2.5, y = 1.7, label = "FDR < 0.1", color = "red", size = 5)
g1
...
legend1
ggsave("legend1_cazy.png", legend1, width = 5, height = 1)
...
legend2
ggsave("legend2_cazy.png",legend2,width = 5, height = 1)
...
pick_fid <- rownames(p.vals_all)[which(p.vals_all$padj < 0.1)]
df_auc <- df_auc %>% filter(fid %in% pick_fid)
cazy_name <- c("Carbohydrate-Binding Module \nFamily 6", "Glycoside Hydrolase \nFamily 27",
               "Glycoside Hydrolase \nFamily 28", "Glycoside Hydrolase \nFamily 6")
names(cazy_name) <- unique(df_auc$fid)
df_auc$cazy <- cazy_name[df_auc$fid]
...
g <- df_auc %>% 
  ...
  facet_grid(~cazy, scales = 'free_x', space='free', switch = "x") +
...
ggsave(p, filename = "Figure_supplement_cazy.pdf",width = 5, height = 8, useDingbats=FALSE)
