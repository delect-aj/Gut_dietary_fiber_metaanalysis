...
abundance_table <- read.csv("../picrust2/KO_metagenome_out/pred_metagenome_unstrat.tsv", 
...
for (i in  study){
  ...
  write.csv(res, paste0("linda_single_study_ko_", i, ".csv"))
}
...
df_list <- list()
for (i in study){
  df <- read.csv(paste0("linda_single_study_ko_", i, ".csv"))
  df$study <- i
  df_list[[i]] <- df
}
...
write.csv(per_otu, paste0("combine_results_ko.csv"))
